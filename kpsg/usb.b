...
# Start eBPF
BPF_LDX_MEM(BPF_W, BPF_REG_6, BPF_REG_1, offset(struct __lbm_user, setup_packet)),
BPF_JMP_IMM(BPF_JNEQ, BPF_REG_6, 0, 2),			/* if setup_packet != null: goto PC+2; */
...
BPF_ALU64_IMM(BPF_SUB, BPF_REG_3, 3),			/* 3  bytes */
...
BPF_MOV64_IMM(BPF_REG_0, 0),                            /* else: allow the pkt */
BPF_EXIT_INSN(),					/* ret 0 */
BPF_JMP_IMM(BPF_JLE, BPF_REG_6, 0x3, 2),		/* if desc_type <= 0x3: goto PC+2; */
BPF_MOV64_IMM(BPF_REG_0, 0),                            /* else: allow the pkt */
BPF_EXIT_INSN(),					/* ret 0 */

/* make sure we have a valid response payload */
BPF_LDX_MEM(BPF_W, BPF_REG_7, BPF_REG_1, offset(struct __lbm_user, actual_length)),
...
BPF_MOV64_REG(BPF_REG_3, BPF_REG_10),			/* to: stack allocation */
BPF_ALU64_IMM(BPF_SUB, BPF_REG_3, 1),			/* 1 byte */
...
BPF_JMP_REG(BPF_JNEQ, BPF_REG_6, BPF_REG_8, 2),		/* if desc_type != bDescriptorType: goto PC+2 */
BPF_MOV64_IMM(BPF_REG_0, 0),                            /* else: allow the pkt */
BPF_EXIT_INSN(),					/* ret 0 */
